...
.env файл:
	...
	# Ollama облако (если нет локального сервера):
...
# Ollama — хост, модель и опциональный API-ключ для облака
...
def generate_post(past_posts: list[str], repo_info: dict | None, commits: list[str]) -> str:
	...
	if is_local:
		# Локальный Ollama — используем OllamaManager (автозапуск + autopull)
		with OllamaManager(
			model=OLLAMA_MODEL,
			host=OLLAMA_HOST,
		...
		# Облако — используем Client напрямую с API-ключом
